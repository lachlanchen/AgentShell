...
If Codex intermittently reports `workspace routing discovery timed out` before opening a session, see [bounded startup retries](codex-startup-retry.md). This Linux/WSL guard preserves the selected account and does not reset logins or session history.
